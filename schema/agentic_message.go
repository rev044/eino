...
const (
	FunctionToolResultContentBlockText  FunctionToolResultContentBlockType = "text"
	FunctionToolResultContentBlockImage FunctionToolResultContentBlockType = "image"
	FunctionToolResultContentBlockAudio FunctionToolResultContentBlockType = "audio"
	FunctionToolResultContentBlockVideo FunctionToolResultContentBlockType = "video"
	FunctionToolResultContentBlockFile  FunctionToolResultContentBlockType = "file"
)
...
func (b *FunctionToolResultContentBlock) String() string {
	switch b.Type {
	case FunctionToolResultContentBlockText:
		if b.Text != nil {
			return b.Text.String()
		}
		return "empty text block\n"
	case FunctionToolResultContentBlockImage:
		if b.Image != nil {
			return b.Image.String()
		}
		return "empty image block\n"
	case FunctionToolResultContentBlockAudio:
		if b.Audio != nil {
			return b.Audio.String()
		}
		return "empty audio block\n"
	case FunctionToolResultContentBlockVideo:
		if b.Video != nil {
			return b.Video.String()
		}
		return "empty video block\n"
	case FunctionToolResultContentBlockFile:
		if b.File != nil {
...
func FunctionToolResultAgenticMessage(callID, name string, result []*FunctionToolResultContentBlock) *AgenticMessage {
	return &AgenticMessage{
		Role: AgenticRoleTypeUser,
...
func concatFunctionToolResults(results []*FunctionToolResult) (*FunctionToolResult, error) {
	...
	var allBlocks []*FunctionToolResultContentBlock
	var hasStreamingMeta bool
	...
	for _, r := range results {
		...
		for _, b := range r.Content {
			...
			hasMeta := b.StreamingMeta != nil
			if !metaChecked {
				hasStreamingMeta = hasMeta
				metaChecked = true
			} else if hasMeta != hasStreamingMeta {
				if hasStreamingMeta {
					return nil, fmt.Errorf("inconsistent streaming meta in function tool result content blocks: " +
						"all blocks must have streaming meta when the first block has it, but found block without streaming meta")
				}
				return nil, fmt.Errorf("inconsistent streaming meta in function tool result content blocks: " +
					"all blocks must omit streaming meta when the first block omits it, but found block with streaming meta")
			}
			allBlocks = append(allBlocks, b)
		}
	}

	if len(allBlocks) == 0 || !hasStreamingMeta {
		ret.Content = allBlocks
		return ret, nil
	}

	merged, err := aggregateBlocksByStreamingMeta(allBlocks)
	if err != nil {
		return nil, err
	}
	ret.Content = merged
...
func aggregateBlocksByStreamingMeta(blocks []*FunctionToolResultContentBlock) ([]*FunctionToolResultContentBlock, error) {
	type group struct {
	...
	var ret []*FunctionToolResultContentBlock
	for idx := 0; idx <= maxIndex; idx++ {
		...
		merged, err := mergeBlockGroup(idx, g.blocks)
		if err != nil {
			return nil, err
		}
...
func mergeBlockGroup(streamIndex int, blocks []*FunctionToolResultContentBlock) (*FunctionToolResultContentBlock, error) {
	first := blocks[0]

	if first.Type == FunctionToolResultContentBlockText {
		merged := &FunctionToolResultContentBlock{
			Type:          FunctionToolResultContentBlockText,
			Text:          &UserInputText{},
			StreamingMeta: first.StreamingMeta,
		}
		var extras []map[string]any
		for _, b := range blocks {
			if b.Type != FunctionToolResultContentBlockText {
				return nil, fmt.Errorf("streaming meta index %d has mixed types: expected %q but got %q",
					streamIndex, FunctionToolResultContentBlockText, b.Type)
			}
			if b.Text != nil {
				merged.Text.Text += b.Text.Text
			}
			if len(b.Extra) > 0 {
		...
		if len(extras) > 0 {
			extra, err := internal.ConcatItems(extras)
			if err != nil {
				return nil, fmt.Errorf("streaming meta index %d: failed to concat extras: %w", streamIndex, err)
			}
			merged.Extra = extra
		}
		return merged, nil
	...
	if len(blocks) > 1 {
		return nil, fmt.Errorf("streaming meta index %d has %d blocks of type %q, "+
			"only one block is allowed for non-text types", streamIndex, len(blocks), first.Type)
	}
